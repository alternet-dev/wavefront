...
// against that status's bound error message. acme.v1.Item has a `text` field,
// so renaming the upstream `detail` leaf onto `text` is a legal response rename
// (rename.to is cross-checked against the bound error message, not the 2xx
// response_message).
